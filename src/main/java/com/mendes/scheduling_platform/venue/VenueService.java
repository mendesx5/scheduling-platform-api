...
public class VenueService {
    ...
    public Venue save(Long t,Venue v){plans.assertCanCreateVenue(t);if(v.getPricingType()==Venue.PricingType.PACKAGE)plans.assertFeatureEnabled(t,PlanService.Feature.PACKAGES);validate(v);normalize(v);v.setId(null);v.setTenantId(t);return repo.save(v);}
    public Venue update(Long t,Long id,Venue in){if(in.getPricingType()==Venue.PricingType.PACKAGE)plans.assertFeatureEnabled(t,PlanService.Feature.PACKAGES);validate(in);Venue v=get(t,id);copy(v,in);normalize(v);return repo.save(v);}
